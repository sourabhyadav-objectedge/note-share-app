...
const deleteNote = createAsyncThunk<
  ...
  { rejectValue: number }
>("user/deleteNote", async ({ _id, index }, thunkApi) => {
  try {
...
const updateNote = createAsyncThunk<
  number,
  { _id: string; note: string | undefined; index: number },
  { rejectValue: number }
>("user/updateNote", async ({ _id, note, index }, thunkApi) => {
  try {
...
const crudCardSlice = createSlice({
  ...
  reducers: {
    ...
    setDeleted(
      state: InitialState,
      { payload }: PayloadAction<InitialState["deleted"]>
    ) {
      state.deleted = payload;
    },
    setDeleteErrorByIndex(
      state: InitialState,
      { payload }: PayloadAction<{ index: number; value: boolean }>
    ) {
    ...
    setUpdated(
      state: InitialState,
      { payload }: PayloadAction<InitialState["updated"]>
    ) {
      state.updated = payload;
    },
    setUpdateErrorByIndex(
      state: InitialState,
      { payload }: PayloadAction<{ index: number; value: boolean }>
    ) {
