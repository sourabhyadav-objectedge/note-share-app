...
export default function NewNote()
{
    ...
    if(status==='authenticated')
        return<>
                <h2 style={{textAlign:"center",marginTop:"2rem"}}>Create your note</h2>
                <hr style={{marginBottom:"2rem"}}/>
                 <Form style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:"2rem"}}>
                    ...
                    <Form.Control as="textarea" style={{width:"80%",fontSize:"1.5rem"}}   rows={6}  value={state.note}  onChange={createNote}/>
                    <Button type="submit" style={{height:"4rem",fontSize:"1.5rem"}} onClick={createNewNote}>Create new note</Button>
                </Form>
                {state.submitting&&<h2 style={{textAlign:"center"}}>Submitting...</h2>}
                {state.error&&<h2 style={{textAlign:"center"}}>An error occured</h2>}
                {state.submitted&&<h2 style={{textAlign:"center"}}>Submitted.</h2>}
            </>
    else return <h2 style={{textAlign:"center"}}>You need to be logged in to create your own notes</h2>
}